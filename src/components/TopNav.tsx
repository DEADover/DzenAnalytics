...
export function TopNav({ onOpenPalette }: { onOpenPalette?: () => void }) {
  ...
  return (
    ...
    >
      ...
      {/* Подложка — не на самой шапке, а на её строке: панель фильтров ниже
          такая же и должна выглядеть так же. Пока фон с размытием стоял на
          `<header>`, размытие панели внутри брало фоном уже его, а не страницу,
          и панель выходила заметно прозрачнее.

          Подложка НЕПРОЗРАЧНАЯ: сквозь полупрозрачную читались суммы и подписи
          уезжающей под шапку таблицы. Размытия поэтому тоже нет — размывать
          нечего. Заодно шапка перестала быть системой отсчёта для `fixed`
          внутри неё. */}
      <div className="w-full px-4 md:px-6 py-3 flex items-center gap-2 sm:gap-3 md:gap-6 border-b border-border bg-panel">
