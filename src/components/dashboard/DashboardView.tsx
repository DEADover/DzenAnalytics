  ...
  return (
    ...
    <div className={clsx("flex flex-col h-full gap-5 justify-between", !sunken && "pb-2.5")}>
      ...
      >
        ...
        {(() => {
          const ahead = m.upcomingTotalBase > 0;
          const sum = (
          ...
          if (m.free.value >= 0) {
            return ahead ? (
              <>Столько остаётся после уже потраченного и ещё {sum}, которые спишутся до конца месяца</>
            ) : (
              <>Столько остаётся после уже потраченного</>
            );
          }
          if (m.factExpense > m.factIncome) {
            return ahead ? (
              <>Столько не хватает: расход месяца уже обогнал доход, и впереди ещё {sum}</>
            ) : (
              <>Столько не хватает: расход месяца уже обогнал доход</>
            );
          }
          return <>Столько не хватает: до конца месяца спишется ещё {sum} — больше, чем осталось</>;
        })()}
