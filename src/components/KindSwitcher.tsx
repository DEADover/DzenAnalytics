 ...
 *
 * `sm` — исходный, компактный: в «Категориях» он стоит в шапке карточки над
 * крупной суммой. `md` — по высоте ровно как `Segmented` обычного размера: в
 * «Топе» оба переключателя стоят в одной строке, и разная высота бросалась в
 * глаза.
 */
const SIZES = {
  sm: { track: "h-7 p-0.5 text-xs", button: "w-[76px] h-6", pill: "top-0.5 bottom-0.5 left-0.5 w-[76px]", shift: 76 },
  md: { track: "p-1 text-[13.5px]", button: "w-[96px] h-8", pill: "top-1 bottom-1 left-1 w-[96px]", shift: 96 },
} as const;
