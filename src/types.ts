...
export interface Transaction {
  ...
  unseen?: boolean;
  /**
   * Чей это объект в Дзен-мани (`ZenTransaction.user`).
   *
   * На личном аккаунте всегда один и тот же, поэтому раньше поле выбрасывали
   * при загрузке. На общем аккаунте по одному токену приезжают операции всех
   * подключённых людей, и без этого поля отличить их было нельзя — issue #92.
   *
   * `undefined` у операций из CSV: там такого понятия нет.
   */
  user?: number;
}
