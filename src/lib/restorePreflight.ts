...
/** Русское склонение для счётных строк препятствий. */
...
export function restorePreflight(
  ...
  if (transactions.tombstoned > 0) {
    ...
    blockers.push({
      ...
      count: n,
      text:
        `${n} ${plural(n, "операция", "операции", "операций")} из снимка ` +
        `${plural(n, "удалена", "удалены", "удалены")} в Дзен-мани — ` +
        `${plural(n, "она не вернётся", "они не вернутся", "они не вернутся")}.`,
      fix:
  ...
  if (transactions.newerInCloud > 0) {
    ...
    blockers.push({
      ...
      count: n,
      text:
        `${n} ${plural(n, "операция", "операции", "операций")} ` +
        `${plural(n, "правилась", "правились", "правились")} после снимка — ` +
        `${plural(n, "останется", "останутся", "останутся")} в нынешнем виде.`,
      fix:
  ...
  if (tags.extra > 0) {
    blockers.push({
      kind: "extraTags",
      count: tags.extra,
      text: `${tags.extra} ${plural(tags.extra, "категория", "категории", "категорий")} есть в аккаунте, но нет в снимке.`,
  ...
  if (merchants.extra > 0) {
    blockers.push({
      ...
      text: `${merchants.extra} ${plural(merchants.extra, "контрагент", "контрагента", "контрагентов")} есть в аккаунте, но нет в снимке.`,
