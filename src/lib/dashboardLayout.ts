...
export const WIDGETS: readonly WidgetMeta[] = [
  ...
  },
  {
    ...
    // Во всю ширину, и не только ради простора: главная собрана в ровные ряды
    // по три, и виджет в одну колонку оставил бы в них дырку у ВСЕХ — ровно то,
    // из-за чего «Месяц к месяцу» пришлось убрать с глаз. Полосой он встаёт
    // своим рядом, ряды сходятся, а кольцо, число и разбивка помещаются в строку
    // и читаются слева направо, как предложение.
    span: 3,
    // Высота по содержимому: у виджетов главной она общая — 30rem, — и полоса
    // в три колонки текста оставляла под собой полэкрана пустоты.
    autoHeight: true,
  },
  {
    kind: "links",
