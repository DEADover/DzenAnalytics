...
describe("chunk", () => {
  ...
  it("категории уходят по одной", () => {
    // Стоимость запроса растёт как КВАДРАТ числа категорий в нём: замерено на
    // живом аккаунте — 1 шт 1,5 с, 5 шт 48 с, 10 шт 177 с. Значит партия тем
    // выгоднее, чем она меньше, и меньше единицы не бывает. Полсотни
    // категорий занимают около минуты вместо восьми.
